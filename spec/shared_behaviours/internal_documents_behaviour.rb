RSpec.shared_examples "internal documents" do
  feature "internal document management", :js => true do
    ...
    scenario "add a new document" do
      expect(page_heading).to eq "#{heading_prefix} Internal Documents"
      expect(page_title).to eq "#{heading_prefix} Internal Documents"
      ...
      expect{upload_files_link.click; sleep(0.5)}.to change{document_model.count}.from(2).to(3)
      expect(page).to have_css(".files .template-download", :count => 2)
      doc = document_model.last
    ...
    scenario "add a new document but omit document name" do
      ...
      expect{upload_files_link.click; sleep(0.5)}.to change{document_model.count}
    ...
    scenario "upload an unpermitted file type" do
      ...
      expect{ upload_files_link.click; sleep(0.5)}.not_to change{document_model.count}
    end
    ...
    scenario "delete a file from database" do
      ...
      sleep(0.3)
      expect(document_model.count).to eq 1
    end
    ...
    scenario "delete the last file in a document group" do
      ...
      page.find('.template-download .delete').click
      sleep(0.3)
      expect(document_model.count).to eq 0
    ...
    feature "add a new revision" do
      context "including user configured title" do
        ...
        scenario "using the single-upload icon" do
          expect{upload_replace_files_link.click; sleep(0.5)}.to change{document_model.count}.by(1)
        ...
        scenario "using the buttonbar upload icon" do
          expect{upload_files_link.click; sleep(0.5)}.to change{document_model.count}.by(1)
          expect(page.all('.template-download').count).to eq 1
          expect(page.find('.template-download .internal_document .title .no_edit span').text).to eq "some replacement file name"
          expect(page.find('.template-download .internal_document .revision .no_edit span').text).to eq "3.5"
        end
    ...
    scenario "add a new file and then add a new revision to it" do
      unless ie_remote?(page)
        ...
        expect{ page.find('.template-upload .start .fa-cloud-upload').click; sleep(0.5)}.to change{document_model.count}
    ...
    scenario "upload a revision then edit the title and revision" do
      ...
      expect{ click_edit_save_icon(page); sleep(0.5)}.not_to change{document_model.count}
      expect(page.all('.template-download').count).to eq 1
    end
    ...
    scenario "delete an archive file" do
      visit index_path
      click_the_archive_icon
      expect{ click_the_archive_delete_icon; sleep(0.2) }.to change{ document_model.count }.by -1
    ...
    describe "add multiple primary files" do
      ...
      scenario "upload with buttonbar action" do
        expect{upload_files_link.click; sleep(0.5)}.to change{document_model.count}.by(3)
        persisted_titles = document_model.all.map(&:title)
      ...
      scenario "upload with multiple individual actions" do
        expect{ upload_single_file_link_click(:first) ; sleep(0.4) }.to change{ document_model.count }.by(1)
        expect{ upload_single_file_link_click(:second); sleep(0.4) }.to change{ document_model.count }.by(1)
        expect{ upload_single_file_link_click(:third) ; sleep(0.4) }.to change{ document_model.count }.by(1)
      end
    end
    ...
    scenario "delete primary file while archive files remain" do
      ...
      page.find("#internal_document_title").set("some replacement file name")
      page.find('#internal_document_revision').set("3.5")
      expect{upload_replace_files_link.click; sleep(0.5)}.to change{document_model.count}.from(2).to(3)
      expect(page.all('.template-download').count).to eq 1
    end
  ...
  feature "behaviour with multiple primary files on the page", :js => true do
    ...
    it "add multiple revisions to different primary files" do
      expect{upload_files_link.click; sleep(1.0)}.to change{document_model.count}.from(2).to(4)
      expect(page.all('.template-download').count).to eq 2
      sleep(3.5)
  ...
  feature "sequential operations", :js => true do
    ...
    it "should correctly follow the sequence of operations" do # b/c there was a bug!
      expect(document_model.count).to eq 5
      #delete a primary that has some archive files
      expect{page.find('.template-download .delete').click; sleep(0.5)}.to change{document_model.count}.by(-1)
      click_the_archive_icon
      expect(page).to have_selector('table.internal_document.editable_container', :count => 4)
      #again delete a primary file
      expect{page.all('.template-download .delete')[0].click; sleep(0.5)}.to change{document_model.count}.by(-1)
