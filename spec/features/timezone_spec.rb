...
feature "complaints index", :js => true do
  ...
  it "adds a new complaint that is valid" do
    ...
    puts "on the client #{page.execute_script('console.log(new Date)')}"

    # on the server
    puts "on server, first status change is:  #{ActiveRecord::Base.connection.execute('select created_at from complaint_statuses').to_a.last["created_at"]}"

    # on the client
    puts Date.today.strftime("%b %e, %Y")
