...
# required by passenger on this server
set :linked_files, fetch(:linked_files, []).push('public/.htaccess', 'config/deploy/nhri-hub.com.rb', 'config/initializers/action_mailer.rb')
set :linked_files, fetch(:linked_files, []).delete_if{|filename| filename=~/config\/deploy\/production\.rb/}
set :assets_roles, fetch(:asset_roles, []).push('demo') # triggers asset precompilation if necessary
