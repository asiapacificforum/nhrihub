namespace :authengine do
  desc 'create an admin access account, only when there are no accounts already configured. Call rake "authengine:bootstrap[firstname,lastname,login,password]" (no spaces, quote the entire command)'
  task :bootstrap, [:firstName, :lastName, :login, :password] => :environment do |t, args|
    if args.to_hash.keys.length < 4
      ...
      if User.count > 0
        ...
        if id = User.create_by_sql(attributes) # postgresql returns the id, not the model!
