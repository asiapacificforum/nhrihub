class Authengine::ActionsController < ApplicationController
  ...
  def index
    ...
    @actions = Action.includes(:controller).all.sort
    @roles = Role.except_developer.includes(:actions=>:controller).order(:name) # this eager loading seems to produce a large number of database accesses, and I'm not sure why!!
    @allowed = []
