class MediaAppearanceMetrics < Hash
  ...
  def initialize(source)
    @source = source
    super[:positivity_rating] = {:rank => send(:positivity_rating_rank), :name => metric_name(:positivity_rating), :value => send(:positivity_rating_rank_text), :id => send(:positivity_rating_id)}
    super[:violation_severity] = {:rank => send(:violation_severity_rank), :name => metric_name(:violation_severity), :value => send(:violation_severity_rank_text), :id => send(:violation_severity_id)}
