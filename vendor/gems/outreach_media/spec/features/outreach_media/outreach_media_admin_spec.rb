...
feature "media admin", :js => true do
  ...
  scenario "add filetype" do
    ...
    expect{ new_filetype_button.click; sleep(0.2) }.to change{ SiteConfig['outreach_media.filetypes'] }.from([]).to(["docx"])
    expect(page).not_to have_selector '#empty'
    page.find('#outreach_media_filetype_ext').set('ppt')
    expect{ new_filetype_button.click; sleep(0.2) }.to change{ SiteConfig['outreach_media.filetypes'].length }.from(1).to(2)
  end

  scenario "add duplicate filetype" do
    SiteConfig['outreach_media.filetypes']=["pdf", "doc"]
    visit outreach_media_admin_path('en')
    sleep(0.1)
    page.find('#outreach_media_filetype_ext').set('doc')
    expect{ new_filetype_button.click; sleep(0.2) }.not_to change{ SiteConfig['outreach_media.filetypes'] }
  ...
  scenario "delete a filetype" do
    ...
    delete_filetype("pdf")
    sleep(0.2)
    expect( SiteConfig['outreach_media.filetypes'] ).to eq ["doc"]
    delete_filetype("doc")
    sleep(0.2)
    expect( SiteConfig['outreach_media.filetypes'] ).to eq []
    expect(page).to have_selector '#empty'
  end
  ...
  scenario "change filesize" do
    ...
    expect{ page.find('#change_filesize').click; sleep(0.2)}.
      to change{ SiteConfig['outreach_media.filesize']}.to(22)
...
feature "media admin when user not permitted", :js => true do
  ...
  scenario "update filetypes" do
    ...
    expect{ new_filetype_button.click; sleep(0.2) }.
      not_to change{ SiteConfig['outreach_media.filetypes'] }
  ...
  scenario "change filesize" do
    ...
    set_filesize("22")
    expect{ page.find('#change_filesize').click; sleep(0.2)}.
      not_to change{ SiteConfig['outreach_media.filesize']}
    expect( flash_message ).to eq "You don't have permission to complete that action."
