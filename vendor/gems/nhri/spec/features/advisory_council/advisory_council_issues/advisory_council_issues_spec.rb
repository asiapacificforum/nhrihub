...
feature "attempt to save with errors", :js => true do
  ...
  feature "neither link nor file is included" do
    before do
      fill_in("advisory_council_issue_title", :with => "My new article title")
      expect{edit_save.click; sleep(0.4)}.not_to change{Nhri::AdvisoryCouncil::AdvisoryCouncilIssue.count}
      expect(page).to have_selector('#advisory_council_issue_error', :text => "A file or link must be included")
    end

    scenario "remove error by adding a file" do
      page.attach_file("advisory_council_issue_file", upload_document, :visible => false)
      expect(page).not_to have_selector('#advisory_council_issue_error', :text => "A file or link must be included")
    end
  ...
  feature "both link and file are included" do
    before do
      ...
      fill_in("advisory_council_issue_article_link", :with => "h")
      expect{edit_save.click; sleep(0.4)}.not_to change{Nhri::AdvisoryCouncil::AdvisoryCouncilIssue.count}
      expect(page).to have_selector('#advisory_council_issue_attachment_error', :text => "Either file or link, not both")
    end

    scenario "remove error by removing file" do
      clear_file_attachment
      expect(page).not_to have_selector('#advisory_council_issue_attachment_error', :text => "Either file or link, not both")
    end
...
feature "when there are existing articles", :js => true do
  ...
  feature "and existing article has file attachment" do
    before do
      setup_database(:advisory_council_issue_with_file)
      setup_file_constraints
      resize_browser_window
      visit nhri_advisory_council_issues_path(:en)
    end
    ...
    scenario "edit an article and upload a different file" do
      ...
      previous_file_id = page.evaluate_script("advisory_council_issues.findAllComponents('ma')[0].get('file_id')")
      ...
      new_file_id = page.evaluate_script("advisory_council_issues.findAllComponents('ma')[0].get('file_id')")
  ...
  feature "and existing article has link attachment" do
    ...
    scenario "edit a file article and change to file" do
      ...
      edit_save.click
      sleep(0.4)
      expect(Nhri::AdvisoryCouncil::AdvisoryCouncilIssue.first.article_link).to be_blank
